...
function real_estate_dark_house_ocdi_plugin_intro_text() {
	?>
	<div class="ocdi__intro-text notice notice-warning" style="background: #fefbc1;">
		<p class="about-description" style="margin: 0;padding: 10px 2px;color: #000;font-weight: 500;">
			<?php esc_html_e( "Importing demo data (post, pages, images, theme settings, etc.) is the quickest and easiest way to set up your new theme.", "real-estate-dark-house" );?><br>
			<?php esc_html_e( "Download our demo content, then follow bellow page 'Manual Demo File Import' and select our XML file.", "real-estate-dark-house" );?><br><br>
			<a class="button-primary" href="//wpdirectorykit.com/demo_themes/download_theme.php?theme=real-estate-dark-house" download="filename"><?php esc_html_e( "Download demo content XML file", "real-estate-dark-house" );?></a>
		</p>
	</div>
	<?php
  }

  add_filter( 'ocdi/plugin_intro_text', 'real_estate_dark_house_ocdi_plugin_intro_text' );

  function real_estate_dark_house_ocdi_register_plugins( $plugins ) {
	$theme_plugins = [
  ...
  add_filter( 'ocdi/register_plugins', 'real_estate_dark_house_ocdi_register_plugins' );

 /* after import */
function real_estate_dark_house_ocdi_after_import_setup($selected_import) {
    ...
    $results_page_id  = wdk_page_by_title( 'Grid map' );
	...
	if ( true) {
		if($results_page_id) {
			...
			$to = real_estate_dark_house_url_suffix(get_permalink($results_page_id), 'is_featured=on#results');
			real_estate_dark_house_replace_links($from, $to);
		}
		
		if($page_for_posts_id) {
			$from = get_home_url().'/blog/';
		...
		if($results_page_id) {
			...
			$to = real_estate_dark_house_url_suffix(get_permalink($results_page_id), 'field_5=Sale#results');
			real_estate_dark_house_replace_links($from, $to);
		}
    }
	...
	set_theme_mod( 'footer_content', esc_html__('80 Broklyn Golden Street, New York. USA', 'real-estate-dark-house') );
	set_theme_mod( 'footer_phone_number', '(917) 382-2057' );
	set_theme_mod( 'footer_email_address', 'needhelp@example.com' );
	set_theme_mod( 'footer_copyright_text', 'Copyright © '.date('Y').' Real Estate Dark House' );
	...
	if(true){
		...
		real_estate_dark_house_insert_widget('sidebar-1', 'search');
		real_estate_dark_house_insert_widget('sidebar-1', 'recent-posts');
		real_estate_dark_house_insert_widget('sidebar-1', 'categories');
		...
		real_estate_dark_house_insert_widget('sidebar', 'search');
		real_estate_dark_house_insert_widget('sidebar', 'recent-posts');
		real_estate_dark_house_insert_widget('sidebar', 'categories');
		...
		real_estate_dark_house_insert_widget('footer', 'text', array('title' => esc_html__('Popular Properties', 'real-estate-dark-house'), 'text'=>'[wdk-latest-listings-list]'));
		real_estate_dark_house_insert_widget('footer', 'recent-posts');
		real_estate_dark_house_insert_widget('footer', 'text', array('title' => esc_html__('Newsletter', 'real-estate-dark-house'), 'text'=>'[eli-newsletter]'));
	}
	...
	if(true){
		...
		set_theme_mod('sign_in_button_text', esc_html__('Login', 'real-estate-dark-house'));
        set_theme_mod('property_button_text', esc_html__('Add Property', 'real-estate-dark-house'));
	}
	update_option('real_estate_dark_house_install_ocdi_images_sizes_enable', 0);

    // Save searchform_m
    $data = array();
    $data['resultitem_name'] = 'Primary';
    $data['resultitem_json'] = '[[{"field_id":"location_id","class":"","columns":""}],[{"field_id":"5","class":"","columns":""}],[{"field_id":"post_title","class":"","columns":""}],[{"field_id":"address","class":"","columns":""},{"field_id":"6","class":"","columns":""}],[{"field_id":"8","class":"","columns":""},{"field_id":"12","class":"","columns":""},{"field_id":"13","class":"","columns":""},{"field_id":"14","class":"","columns":""},{"field_id":"64","class":"","columns":""},{"field_id":"22","class":"","columns":""}],[]]';
    $data['idresultitem'] = 1; 
    $data['is_multiline_enabled'] = 1; 
    ...
    $data = array(
        'field_type' => 'INPUTBOX',
        'field_label' => esc_html__('Check In', 'real-estate-dark-house'),
    ...
    $data = array(
        'field_type' => 'INPUTBOX',
        'field_label' => esc_html__('Check Out', 'real-estate-dark-house'),
        'is_visible_frontend' => '1',
        'is_visible_dashboard' => '1', 
        'order_index' =>  64,
    );
    ...
    $data = array(
        ...
        'field_label' => esc_html__('About Unit', 'real-estate-dark-house'),
        'is_visible_frontend' => '1',
        'is_visible_dashboard' => '1', 
        'order_index' =>  2,
    );
    ...
    $data = array(
        'suffix' => esc_html__('/Night', 'real-estate-dark-house'),
    );
    $insert_id = $WMVC->field_m->insert($data, 36);
    ...
    update_option('wdk_theme_real_estate_dark_house_installed', 1);
}

function real_estate_dark_house_replace_links($from = '', $to = '') {
	global $wpdb;
...
add_action( 'ocdi/after_import', 'real_estate_dark_house_ocdi_after_import_setup' );

if(!function_exists('real_estate_dark_house_insert_widget'))
{
    function real_estate_dark_house_insert_widget($sidebar_id, $widget_name, $widget_options_new = array())
    {
...
if ( ! function_exists('real_estate_dark_house_url_suffix'))
{
	function real_estate_dark_house_url_suffix($base_url, $extension_url="")
	{
